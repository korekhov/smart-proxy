...
module Proxy::DHCP::CommonISC
  class IscOmapiProvider < ::Proxy::DHCP::Server
    ...
    def om_add_record(record)
      ...
      omcmd "set group = \"#{record.group}\"" if record.group
      ...
      options.each do |key, value|
        case key
          when "domain-name"
            statements << "option domain-name = \\\"#{value}\\\";"
          when "domain-name-servers"
            ip = validate_ip(value)
            statements << "option domain-name-servers = \\\"#{ip}\\\";"
          when "ntp-servers"
            statements << "option ntp-servers = \\\"#{value}\\\";"
          when "routers"
            ip = validate_ip(value)
            statements << "option routers = \\\"#{ip}\\\";"
          when "broadcast-address"
            ip = validate_ip(value)
            statements << "option broadcast-address = \\\"#{ip}\\\";"
          when "subnet-mask"
            ip = validate_ip(value)
            statements << "option subnet-mask = \\\"#{ip}\\\";"
        end
      end
